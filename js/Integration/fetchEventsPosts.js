...
async function fetchEventsPosts(){
    ...
    if(checkFetchedData.faculty === "Faculty of Business"){
        response = await fetch("http://localhost:5000/getPostsByCategoryAndFaculty/Events/Business")
    }

    else if(checkFetchedData.faculty === "Faculty of IT"){
        response = await fetch("http://localhost:5000/getPostsByCategoryAndFaculty/Events/IT")
    }

    else {
        response = await fetch("http://localhost:5000/getPostsByCategory/Events")        
    }
