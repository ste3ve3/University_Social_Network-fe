...
async function fetchEventsPosts(){
    ...
    if(checkFetchedData.faculty === "Faculty of Business"){
        response = await fetch("https://university-social-network-be.herokuapp.com/getPostsByCategoryAndFaculty/Events/Business")
    }

    else if(checkFetchedData.faculty === "Faculty of IT"){
        response = await fetch("https://university-social-network-be.herokuapp.com/getPostsByCategoryAndFaculty/Events/IT")
    }

    else {
        response = await fetch("https://university-social-network-be.herokuapp.com/getPostsByCategory/Events")        
    }
    ...
    for(let i=0;i<posts.length;i++){
        let events = document.getElementById("Real");
